...
# Location statuses
location_backend = ap_statuses.Statuses()
...
def _datetime_to_discord(input_time):
...
def _get_status():
    status = location_backend.get_all_statuses()
    broken_locations = []
...
def default_status_update():
    broken_locations = _get_status()
    message = ""
    for key in broken_locations:
        val = location_backend.get_status(key)['verified']
        name = locations[key]['name']
        if val == None:
            message += f"- {name}\n"
        else:
            message += f"- {name} (Verified as of <t:{_datetime_to_discord(val)}>)\n"
    if (len(message) == 0):
        message += "All locations working!"
    return message

# helper function for updating full status
def full_status_update(embed):
    status = location_backend.get_all_statuses()
    for key, val in status.items():
        if val['is_working'] == True:
            if val['verified'] == None:
                ...
                time = _datetime_to_discord(val['verified'])
                embed.add_field(name=locations[key]['name'], 
                                value=f"✅\nVerified at <t:{_datetime_to_discord(time)}>", inline=True)
        else:
            if val['verified'] == None:
                embed.add_field(name=locations[key]['name'], 
                                value="❌", inline=True)
...
class Query(commands.Cog):
    ...
    async def status(self, ctx: discord.ApplicationContext):
        embed = discord.Embed(
            ...
            description=f"Status as of <t:{_get_current_time()}>",
            color=discord.Colour.brand_green() # Pycord provides a class with default colors you can choose from
        )   
        embed.add_field(name="The following lifts/escalators are broken:", 
                        value=default_status_update(), inline=True)
    ...
    async def fullstatus(self, ctx: discord.ApplicationContext):
        ...
        full_status_update(embed)
        await ctx.respond(embed=embed) 

    # Update command takes user arguments and updates the status of the location provided.
    @discord.command()
    @option("location", description="Location and direction of travel to be updated.", required=True, 
            choices=[discord.OptionChoice(locations['mquni-escalator-up']['name'], value='mquni-escalator-up'), 
    ...
    async def update(self, 
        ...
        try:
            ...
            location_backend.set_status(location, is_working, ctx.author)
            
            # Create the response message
            location_name = locations[location]['name']
            embed = discord.Embed(
                title=f'Status update for {location_name}',
                description=f"{status.capitalize()} as of <t:{_get_current_time()}>",
                color=discord.Colour.red()
            )
            # Give user acknowledgement, send the status update message, 
            # add the tick reaction and store the message ID in the locations dict.
